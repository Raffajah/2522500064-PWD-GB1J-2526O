...
if (!$row) {
    $_SESSION['sinar_error'] = "Record tidak ditemukan.";
    redirect_ke('read.php');
}
$nama = $row['cnama'] ?? '';
$email = $row['cemail'] ?? '';
...
$old = $_SESSION['old'] ?? [];
unset($_SESSION['sinar_error'], $_SESSION['old']);
if (!empty($old)) {
    $nama = $old['nama'] ?? $nama;
...
<body>
    ...
    <main>
        <section id="contact">
          ...
          background:#ffd3da; color:#721c24; border-radius:5px;">
                    <?= $sinar_error; ?>
                </div>
            <?php endif; ?>
            ...
            <form action="proses_update.php" method="POST">
                ...
                <label for="captcha"><span>Captcha 2 x 3 = </span>
